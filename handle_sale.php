...
function handleSale($productId, $quantitySold) {
    ...
    try {
        ...
        // Log the movement in `StockMovements`
        ...
        if ($product['StockQuantity'] - $quantitySold < $product['MinimumStock']) {
            $stmt = $conn->prepare("INSERT INTO LowStockAlerts (ProductID, PharmacyID, CurrentStock, Threshold) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("iiii", $productId, $pharmacyID, $product['StockQuantity'] - $quantitySold, $product['MinimumStock']);
            $stmt->execute();
        }

        $conn->commit();
        echo "Sale processed successfully. Remaining stock: " . ($product['StockQuantity'] - $quantitySold);
    } catch (Exception $e) {
        $conn->rollback();
